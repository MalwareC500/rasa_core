...
class MessageProcessor(object):
    ...
    def predict_next(self, sender_id: Text, page_id: Text) -> Optional[Dict[Text, Any]]:

        # we have a Tracker instance for each user
        # which maintains conversation state
        tracker = self._get_tracker(sender_id, page_id)
        if not tracker:
    ...
    def handle_reminder(self,
        ...
        tracker = self._get_tracker(dispatcher.sender_id, dispatcher.page_id)
    ...
    def _get_tracker(self, sender_id: Text, page_id:Text) -> Optional[DialogueStateTracker]:

        sender_id = sender_id or UserMessage.DEFAULT_SENDER_ID
        page_id = page_id or UserMessage.DEFAULT_PAGE_ID
        tracker = self.tracker_store.get_or_create_tracker(sender_id, page_id)
        return tracker
