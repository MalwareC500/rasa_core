...
class TrackerStore(object):
    ...
    def get_or_create_tracker(self, sender_id, page_id, max_event_history=None):
    ...
    def init_tracker(self, sender_id, page_id):
        if self.domain:
    ...
    def create_tracker(self, sender_id, page_id, append_action_listen=True):
        """Creates a new tracker for the sender_id.

        The tracker is initially listening."""

        tracker = self.init_tracker(sender_id, page_id)
        if tracker:
    ...
    def retrieve(self, sender_id: Text, page_id: Text) -> Optional[DialogueStateTracker]:
        raise NotImplementedError()

    def stream_events(self, tracker: DialogueStateTracker) -> None:
        old_tracker = self.retrieve(tracker.sender_id, tracker.page_id)
    ...
    def deserialise_tracker(self, sender_id, page_id, _json):
...
class InMemoryTrackerStore(TrackerStore):
    ...
    def retrieve(self, sender_id: Text, page_id: Text) -> Optional[DialogueStateTracker]:
        if "-".join([page_id, sender_id]) in self.store:
            logger.debug('Recreating tracker for '
                         'id \'{}\''.format(sender_id))
            return self.deserialise_tracker(sender_id, page_id, self.store["-".join([page_id, sender_id])])
        else:
...
class RedisTrackerStore(TrackerStore):
    ...
    def retrieve(self, sender_id, page_id):
        stored = self.red.get(sender_id)
        if stored is not None:
            return self.deserialise_tracker(sender_id, page_id, stored)
        else:
            return None
...
class MongoTrackerStore(TrackerStore):
    ...
    def retrieve(self, sender_id, page_id):
        ...
        # and update them.
        if stored is None and sender_id.isdigit() and page_id.isdigit():
